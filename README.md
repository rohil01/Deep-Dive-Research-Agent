...
If you're explaining this architecture in an interview:

1. **State Machine Pattern**: "It's not a linear chain—it's a cyclic graph with conditional routing"

2. **Separation of Concerns**: "Each node has a single responsibility: planning, research, critique, or writing"

3. **Graceful Degradation**: "The critic ensures quality, but after 3 iterations, it accepts the best available research"

4. **Tool Orchestration**: "The researcher intelligently decides when to scrape vs. when search results suffice"

5. **Scalability**: "Easy to add new nodes (e.g., fact-checker, citation validator) without rewriting logic"
...
**Solution**: The LLM sometimes adds preamble text. The code strips markdown fences, but if errors persist, add more robust parsing:
...
**Solution**: Some sites block automated scraping. The code already handles this gracefully by storing `None` for failed scrapes.
...
- [ ] Add caching layer to avoid re-searching identical queries
- [ ] Implement parallel research for faster execution
- [ ] Add fact-checking node with cross-referencing
